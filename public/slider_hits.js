 ...
 */
 ...
 const SELECTOR_PREV = '.slider__control[data-slide="prevSlide"]';
 const SELECTOR_NEXT = '.slider__control[data-slide="nextSlide"]';
 ...
 class ChiefSlider {
  ...
  _addIndicators() {
    if (this._$root.querySelector(SELECTOR_INDICATOR) !== null || !this._config.indicators) {
      return;
    }
    let html = '';
    for (let i = 0, length = this._$itemList.length; i < length; i++) {
      html += `<li class="${CLASS_NAME_INDICATOR}" data-slide-to="${i}"></li>`;
    }
    this._$root.insertAdjacentHTML('beforeend', `<ol class="${CLASS_NAME_INDICATORS}">${html}</ol>`);
    this._$indicatorList = this._$root.querySelectorAll(SELECTOR_INDICATOR);
  }
   ...
   _move() {
     const step = this._direction === 'nextSlide' ? -this._transformStep : this._transformStep;
   ...
   _moveTo(index) {
     ...
     this._direction = diff > 0 ? 'nextSlide' : 'prevSlide';
