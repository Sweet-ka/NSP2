...
async function addCategories() {
    ...
    activeGroup = location.pathname.match(/\/\w*$/).toString().substring(1);
    ...
    function setActive(el) {
        if(el.getAttribute('data-id') === activeCatID){
            ...
            category_label.innerText = groupName + ' / ' + el.getAttribute('data-category');
            field_display.innerHTML = '';
            for (let elem of arrGoods) {
                if (elem.goodsCategoriesID === Number(activeCatID) || activeCatID === null){
